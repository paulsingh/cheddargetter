...
class CheddarGetter
  ...
  # TODO: need to be able to increment by more than 1, if necessary. (See CG API docs for more info)
  ...
  # By default, this will automatically increment by 1.
  # ODO: need to be able to decrement by more than 1, if necessary. (See CG API docs for more info)
  def decrement_item(item_code, customer_code)
    response = post("/customers/remove-item-quantity/productCode/#{@product_code}/code/#{customer_code}/itemCode/#{item_code}")
    normalize(response, 'customers', 'customer')
  end
